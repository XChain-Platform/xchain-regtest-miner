...
#
# .github/workflows/ci.yml fans this repo out as three jobs (test, drift-guards,
...
# local twin of the workflow: every job's run-steps, transcribed, in job order.
# When ci.yml gains or changes a job, change this script in the same commit.
#
...
# --- job: test (npm ci -> npm run ci) ---------------------------------------
run_tier "test (npm run ci)" npm run ci
