 ...
 *
 *   text       any literal run of `xchain-regtest-miner/src/<path>` in any text file:
 *              a relative require, a comment, a shell script, a markdown runbook.
 *   join       a path built segment by segment, the shape
 *              path.join(root, 'xchain-regtest-miner', 'src', 'foo.js'). When every
 ...
 *              `const R = path.resolve(__dirname, '../../xchain-regtest-miner')` then
 *              path.join(R, 'src', 'foo.js'), path.join(R, 'src/foo.js') or
 *              `${R}/src/foo.js`. The root may equally be
 *              process.env.XCHAIN_REGTEST_MINER_PATH, a candidate array filtered to its
 ...
 *              this checkout, `minerFile('src/rollback.js')`. A call over a
 *              literal array, `minerFile('src/' + twin)`, is emitted once per
 ...
 *              `copy_twin xchain-regtest-miner "src/$f"`, resolved against the literal
...
const REPO_NAME = 'xchain-regtest-miner';
...
// `xchain-regtest-miner/src/<path>`, however it was spelled: a relative require
// (`../../xchain-regtest-miner/src/utility.js`), a prose mention, a shell path.
const TEXT_REFERENCE = new RegExp(`${NAME_RE}\\/(src\\/[A-Za-z0-9_@.\\-/]+)`, 'g');

// path.join(..., 'xchain-regtest-miner', 'src', ...): the tail is captured raw and
...
// Environment variables that hold a checkout of this repo. XCHAIN_REGTEST_MINER_SQL_PATH
...
const ENV_ROOT_VARS = ['XCHAIN_REGTEST_MINER_PATH', 'XCHAIN_REGTEST_MINER_DIR'];
...
function rootPrefix(init) {
    ...
    while ((m = re.exec(init)) !== null) {
        ...
        // path.join(base, 'xchain-regtest-miner', 'src', 'sql').
...
function collectRootProducers(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // `const p = minerFile('src/x.js')` look like another checkout.
 ...
 * path per element: the explorer's hub-mirror guard and a sibling's twin loop
 * are both written that way, and a move has to repoint every element, so
 ...
 * checkout, the `minerFile('src/rollback.js')` idiom. Recognised by a body
 ...
 * array of rows, which is how both SHARED_GATES and a post-move sibling twin
 * loop are written: `[['merkle.js', 'src/consensus/merkle.js'], ...]`.
 */
...
function helperReferences(text, helpers, lists, strings) {
    ...
    for (const [name, suffix] of helpers) {
        ...
        let m;
        while ((m = re.exec(text)) !== null) {
            // `function minerFile(rel){` reads as a call of itself. Its
 ...
 * relative path, `copy_twin xchain-regtest-miner "src/$f"`, where the file comes from
